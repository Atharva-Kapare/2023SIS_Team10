...
function Song( { navigation, track, moodList } ) {

    // const [ playlistMenu, setPlaylistMenu ] = useState(["mood"]);

    // const handleChange = (event) => {
    //     setPlaylistMenu(event)
    // }
        ...
        <div>
            <Box>
                <Grid container justifyContent={'center'} rowSpacing={{ xs: 3, sm: 2, md: 6 }} columnSpacing={{ xs: 2, sm: 1, md: 4 }} columns={{ xs: 2, sm: 6, md: 12 }}>
                    ...
                    {/* more */}
                    <Grid item xs={.2} sm={.5} md={1}>
                        <button className="icon-button-style" onClick={() => {navigation.navigate({
                            name:"TagSongMoodScreen",
                            params: {
                                song: track,
                                moodList: moodList
                            }
                        })}}><img alt="more" className='icon-style' src={MoreIcon}></img></button>
                    </Grid>
                </Grid>
            </Box>
        </div>
